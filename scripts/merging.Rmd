...
# CBCL and BPM
psychopathology_sum_scores <- read_csv("../outputs/psychopathology_sum_scores.csv") %>% dplyr::select(-contains(c("interview" ,"sex")))
```
...
asthma_inflammation_wide <- asthma_inflammation_set %>%
  ...
  # Use the most recent data: household_income at 2-year and parents_avg_edu at 1-year follow-up
  # left_join(demographics_long %>% filter(eventname == "1_year_follow_up_y_arm_1") %>% dplyr::select(src_subject_id, parents_avg_edu)) %>% 
  # left_join(demographics_long %>% filter(eventname == "2_year_follow_up_y_arm_1") %>% dplyr::select(src_subject_id, household_income)) %>% 
  # left_join(site %>% filter(eventname == "baseline_year_1_arm_1") %>% dplyr::select(-eventname)) %>% 
  # left_join(genetic %>% dplyr::select(src_subject_id, asthma_PRS, genetic_afr)) %>% 
