...
## 3.1.6. Sum of number of asthma attacks/sum number of mediation and SA #asthma_num_attack_sum #Asthma_medications_2w_total_sum
```{r}
# Among people with asthma attack only
Noattack_sum_SA_1_1 <- glmer(get_models(outcome = "SA_y_ever", predictor = "asthma_num_attack_sum",
                                    variables = "(1 | site_id_l_br/rel_family_id)"), 
                         family = binomial, data = data_wide %>% filter(asthma_attack_l_ever == 1), nAGQ = 0)

Noattack_sum_SA_1_2 <- glmer(get_models(outcome = "SA_y_ever", predictor = "asthma_num_attack_sum",
                                    variables = var_mod_1), 
                         family = binomial, data = data_wide %>% filter(asthma_attack_l_ever == 1), nAGQ = 0)

Nomedication_sum_SA_1_1 <- glmer(get_models(outcome = "SA_y_ever", predictor = "Asthma_medications_2w_total_sum",
                                        variables = "(1 | site_id_l_br/rel_family_id)"), 
                             family = binomial, data = data_wide %>% filter(asthma_attack_l_ever == 1), nAGQ = 0)

Nomedication_sum_SA_1_2 <- glmer(get_models(outcome = "SA_y_ever", predictor = "Asthma_medications_2w_total_sum",
                                        variables = var_mod_1), 
                             family = binomial, data = data_wide %>% filter(asthma_attack_l_ever == 1), nAGQ = 0)

tab_model(Noattack_sum_SA_1_1, Noattack_sum_SA_1_2, Nomedication_sum_SA_1_1, Nomedication_sum_SA_1_2, show.intercept = F)

# Among people with asthma diagnosis only
Noattack_sum_SA_2_1 <- glmer(get_models(outcome = "SA_y_ever", predictor = "asthma_num_attack_sum",
                                    variables = "(1 | site_id_l_br/rel_family_id)"), 
                         family = binomial, data = data_wide %>% filter(asthma_diagnosis_l_ever == 1), nAGQ = 0)

Noattack_sum_SA_2_2 <- glmer(get_models(outcome = "SA_y_ever", predictor = "asthma_num_attack_sum",
                                    variables = var_mod_1), 
                         family = binomial, data = data_wide %>% filter(asthma_diagnosis_l_ever == 1), nAGQ = 0)

Nomedication_sum_SA_2_1 <- glmer(get_models(outcome = "SA_y_ever", predictor = "Asthma_medications_2w_total_sum",
                                        variables = "(1 | site_id_l_br/rel_family_id)"), 
                             family = binomial, data = data_wide %>% filter(asthma_diagnosis_l_ever == 1), nAGQ = 0)

Nomedication_sum_SA_2_2 <- glmer(get_models(outcome = "SA_y_ever", predictor = "Asthma_medications_2w_total_sum",
                                        variables = var_mod_1), 
                             family = binomial, data = data_wide %>% filter(asthma_diagnosis_l_ever == 1), nAGQ = 0)

tab_model(Noattack_sum_SA_2_1, Noattack_sum_SA_2_2, Nomedication_sum_SA_2_1, Nomedication_sum_SA_2_2, show.intercept = F)

# remove 0 asthma attack (for mod1)
Noattack_sum_SA_3_1 <- glmer(get_models(outcome = "SA_y_ever", predictor = "asthma_num_attack_sum",
                                    variables = "(1 | site_id_l_br/rel_family_id)"), 
                         family = binomial, data = data_wide %>% filter(medhx_ss_6l_times_p_l_ever > 0), nAGQ = 0)

Noattack_sum_SA_3_2 <- glmer(get_models(outcome = "SA_y_ever", predictor = "asthma_num_attack_sum",
                                    variables = var_mod_1), 
                         family = binomial, data = data_wide %>% filter(medhx_ss_6l_times_p_l_ever > 0), nAGQ = 0)

tab_model(Noattack_sum_SA_3_1, Noattack_sum_SA_3_2, show.intercept = F)

# remove 0 medications (for mod2)
Nomedication_sum_SA_3_1 <- glmer(get_models(outcome = "SA_y_ever", predictor = "Asthma_medications_2w_total_sum",
                                        variables = "(1 | site_id_l_br/rel_family_id)"), 
                             family = binomial, data = data_wide %>% filter(Asthma_medications_2w_total_sum > 0), nAGQ = 0)

Nomedication_sum_SA_3_2 <- glmer(get_models(outcome = "SA_y_ever", predictor = "Asthma_medications_2w_total_sum",
                                        variables = var_mod_1), 
                             family = binomial, data = data_wide %>% filter(Asthma_medications_2w_total_sum > 0), nAGQ = 0)

tab_model(Nomedication_sum_SA_3_1, Nomedication_sum_SA_3_2, show.intercept = F)
```
...
```{r}
# diagnosis
diag_med_0 <- glmer(get_models(outcome = "asthma_diagnosis_l_ever", predictor = "Asthma_medications_2w_total_mean", 
                             variables = "(1 | site_id_l_br/rel_family_id)"), 
                      family = binomial, data = data_wide, nAGQ = 0)
...
# composite
composite_med_0 <- glmer(get_models(outcome = "asthma_composite_ever", predictor = "Asthma_medications_2w_total_mean", 
                                 variables = "(1 | site_id_l_br/rel_family_id)"), 
                      family = binomial, data = data_wide, nAGQ = 0)

composite_med_1 <- glmer(get_models(outcome = "asthma_composite_ever", predictor = "Asthma_medications_2w_total_mean", 
                                 variables = var_mod_1), 
                      family = binomial, data = data_wide, nAGQ = 0)

tab_model(composite_med_0, composite_med_1, show.intercept = F)
